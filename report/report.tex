...
\title{Spark-Based Market-Basket Analysis using the Apriori Principle on the IMDB Top 1000 Movies Dataset}
...
\section{Algorithm and Implementation (Spark + Apriori Principle)}

To align with the requirement of applying techniques studied in the course, the mining pipeline is implemented using Apache Spark (PySpark). The algorithmic logic follows the Apriori principle for candidate pruning.

\subsection{Frequent singletons}

First, we compute the support of all individual actors (singletons). This is done by exploding the actor array and using a Spark aggregation:

\texttt{explode(actors) → groupBy(actor).count()}

Actors whose support is greater than or equal to the minimum support threshold are retained as frequent singletons.

\subsection{Apriori candidate pruning}

The Apriori algorithm exploits the monotonicity (downward closure) property: if an itemset is frequent, then all of its subsets must also be frequent.

Using this property, candidate pairs are generated only from actors that are frequent as singletons. In other words, before generating pairs, each basket is filtered so that it contains only frequent actors. This avoids considering pairs that cannot possibly be frequent.

\subsection{Frequent pairs}

After filtering transactions using the frequent singleton set, candidate actor pairs are generated from the remaining actors in each basket. Support for these candidate pairs is then computed using Spark transformations. Candidate pairs are generated within each transaction and their support is aggregated across the dataset using distributed counting operations.

\subsection{Support}

For an itemset $X$, support is defined as the number of transactions containing $X$. In this project we use an absolute minimum support threshold of 2.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\linewidth]{runtime_plot_updated_spark_apriori.png}
...
Additionally, the use of the Apriori principle reduces the search space by generating candidate pairs only from frequent singletons. This pruning step avoids unnecessary pair evaluations and aligns the implementation with the classical market-basket analysis approach.This approach significantly reduces the number of candidate itemsets compared to a brute-force enumeration of all possible pairs.
